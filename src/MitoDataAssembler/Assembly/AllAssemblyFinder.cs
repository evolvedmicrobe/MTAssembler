...
namespace MitoDataAssembler
{
    ...
    /// <summary>
    /// Given a circular genome and a painted deBruijin Graph, attempts to output all assemblies
    /// 
    /// Copied from largeDeletionFinder, should integrate later to avoid duplicate code
    /// </summary>
    class AllAssemblyFinder
    {
        #region Fields, Constructor, Properties

        
        public List<PossibleAssembly> PossibleAssemblies = new List<PossibleAssembly>();
        
        /// <summary>
        /// Initializes a new instance of the deletion finder, has the 
        /// </summary>
        /// <param name="lengthToReport"></param>
        public AllAssemblyFinder()
        {
        }

        #endregion
        private int KmerLength;
        public void OutputAllAssemblies(DeBruijnGraph graph,string fileName)
        {
            ...
            foreach (DeBruijnNode node in graph.GetNodes())
            {
				...
				try
				{
                    PossibleAssemblies.AddRange(ExtendFromStartNode(node));
				}
        ...
        private void Output(string name)
        {
            ...
            {
                var s = new Sequence(DnaAlphabet.Instance, assembly.SequenceAsString, false);
                s.ID = "Contig: " + i.ToString() + " AvgCov = " +assembly.AvgKmerCoverage.ToString()+"; Circle= "+assembly.CircularLoop.ToString()+";";
                s.ID += assembly.FirstReferencePosition.HasValue ? assembly.FirstReferencePosition.Value.ToString() : "No Start";
                s.ID += " - ";
                s.ID += assembly.LastReferencePosition.HasValue ? assembly.LastReferencePosition.Value.ToString() : "No Start";
                i++;
                sequences.Add(s);
            }
            FastAFormatter faf = new FastAFormatter(name);
            faf.Write(sequences);
            faf.Close();
