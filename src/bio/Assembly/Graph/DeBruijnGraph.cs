...
namespace Bio.Algorithms.Assembly.Graph
{
    ...
    public class DeBruijnGraph 
    {
		...
		public KmerDictionary KmerManager {get;private set;}
        /// <summary>
               ...
               this._nodes,
                node =>
                {
                    ...
                    for (ulong i = 0; i < 4; i++)
                    {
                        ...
                        bool matchIsNotRC = searchNodeValue.SetKmerData(tmpNextKmer, this.KmerLength);
                        searchResult = kmerManager.TryGetOld(searchNodeValue);
                        if (searchResult != null)
                        {
                            node.SetExtensionNode(true, matchIsNotRC, searchResult);
                        }
                    }
                    // Left Extensions
                    ...
                    for (ulong i = 0; i < 4; i++)//cycle through A,C,G,T
                    {
                        ...
                        if (searchResult != null)
                        {
                            node.SetExtensionNode(false, matchIsNotRC, searchResult);
                        }
                    }
                });
